...
git hooks on every commit and push, because at the time there was no remote CI
(ADR-0010; Actions came back in ADR-0013 once the repo went public).
...
- **Self-observability starts on day one.** The `ozy.*` registry exists
...
| Git hooks as the only CI | Nothing verifies a PR remotely, and `--no-verify` can bypass everything | Resolved when the repo went public: Actions runs `make ci` on every PR (ADR-0013) |
